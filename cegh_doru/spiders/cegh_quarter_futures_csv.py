...
class CeghCSVQuarterFuturesSpider(scrapy.Spider):
    name = "cegh_quarter_futures_csv"
	
    def start_requests(self):
        url = 'https://www.cegh.at/wp-admin/admin-ajax.php'
        form_data={
            'action': 'get_QuarterlyFutures',
			'period': 'Quarter',
            'product': 'quarterly',
            'market': 'AT'
        }
    ...
    def parse(self, response):
        ...
        item['original_file_name'] = 'AT_futures_Quarter_front-period_1_' + str
        yield item
        ...
        today = date.today()
        str = today.strftime("%d%m%Y")
        item['original_file_name'] = 'AT_futures_Quarter_front-period_2_' + str
        yield item
